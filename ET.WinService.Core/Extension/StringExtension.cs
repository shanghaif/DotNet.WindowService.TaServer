...
namespace ET.WinService.Core.Extension
{
    ...
    {
        private static ILog log = LogManager.GetLogger(typeof(StringExtension));
        ...
        /// <returns></returns>
        public static bool IgnoreCaseEquals(this string text, string value)
        {
        ...
        public static TimeSpan ToTimeSpan(this string text)
        {
            if (string.IsNullOrEmpty(text))
            {
                log.Warn(string.Format("text={0}不是有效的TimeSpan格式，正确格式是{{小时:分钟:秒}}或{{天:小时:分钟:秒}},系统使用3秒做为默认值", text));
